/* ============================================================================
   NexoSoft — Maqueta INTERACTIVA (demo del lado del cliente, sin backend).
...
/* ---------------- Datos (seed) ---------------- */
var STORE_KEY = "nexosoft_demo_v1";
var DB;
...
function seed() {
  return {
    ...
    ventas: [
      ...
      { n: 122, hora: "09:48", items: [{ pid: "p2", q: 1 }, { pid: "p6", q: 2 }, { pid: "p4", q: 1 }], pago: "tarjeta", receptor: "RI" },
      ...
      { n: 126, hora: "12:20", items: [{ pid: "p6", q: 1 }, { pid: "p2", q: 2 }], pago: "ctacte", receptor: "RI" },
...
function load() {
  try { var r = localStorage.getItem(STORE_KEY); if (r) { DB = JSON.parse(r); return; } } catch (e) {}
  DB = seed();
}
...
function ventaTotal(v) {
  return v.items.reduce(function (s, it) { var p = prod(it.pid); var pr = it.precio != null ? it.precio : (p ? p.precioMin : 0); return s + pr * it.q; }, 0);
}
function tipoComprobante(emisor, receptor) {
  if (emisor === "Mono") return "Factura C";
  if (receptor === "RI") return "Factura A";
  return "Factura B";
}
...
function renderInicio() {
  var v = DB.ventas, totalDia = 0, n = v.length, ing = 0, cost = 0, agg = {};
  v.forEach(function (vt) {
    totalDia += ventaTotal(vt);
    vt.items.forEach(function (it) {
      var p = prod(it.pid); if (!p) return;
      var pr = it.precio != null ? it.precio : p.precioMin;
      ing += pr * it.q; cost += p.costo * it.q;
      agg[it.pid] = (agg[it.pid] || 0) + it.q;
    });
  });
  byId("kpiVentas").textContent = money(totalDia);
  byId("kpiComp").textContent = n;
  byId("kpiTicket").textContent = n ? money(totalDia / n) : "$ 0";
  byId("kpiMargen").textContent = ing ? ((ing - cost) / ing * 100).toFixed(1).replace(".", ",") + "%" : "—";
  var top = Object.keys(agg).map(function (k) { return { p: prod(k), q: agg[k] }; }).filter(function (x) { return x.p; })
    .sort(function (a, b) { return b.q - a.q; }).slice(0, 5);
  byId("masVendidos").innerHTML = top.map(function (x) {
    return '<tr><td>' + (x.p.emoji || "📦") + " " + esc(x.p.nombre) + '</td><td class="num strong">' + qtyFmt(x.q) + "</td></tr>";
  }).join("") || '<tr><td class="muted">Sin ventas aún</td><td></td></tr>';
}

/* ---------------- POS ---------------- */
function renderPOS() {
  byId("posCats").innerHTML = ["Todos"].concat(rubros()).map(function (r) {
    return '<button class="cat' + (r === posCat ? " cat--on" : "") + '" onclick="posSetCat(\'' + r + '\')">' + esc(r) + "</button>";
  }).join("");
  var q = (val("posSearch") || "").toLowerCase();
  var list = DB.products.filter(function (p) {
    return (posCat === "Todos" || p.rubro === posCat) && (p.nombre.toLowerCase().indexOf(q) >= 0 || (p.codigo || "").indexOf(q) >= 0);
  });
  byId("posProducts").innerHTML = list.length ? list.map(function (p) {
    var off = p.stock <= 0;
    return '<button class="prod' + (off ? " prod--off" : "") + '" onclick="posAdd(\'' + p.id + '\')">' +
...
function renderTicket() {
  ...
  byId("posItems").innerHTML = items.length ? items.map(function (it) {
    return '<div class="ti"><div class="ti__steps">' +
      '<button class="stepbtn" onclick="posDec(\'' + it.p.id + '\')">–</button>' +
      '<span class="ti__q">' + qtyFmt(it.q) + "</span>" +
      '<button class="stepbtn" onclick="posInc(\'' + it.p.id + '\')">+</button></div>' +
      ...
      '<button class="ti__x" onclick="posRemove(\'' + it.p.id + '\')">×</button></div>';
  }).join("") : '<div class="empty">🛒 Ticket vacío<br><span class="muted">Tocá un producto para agregarlo</span></div>';

  var subtotal = items.reduce(function (s, it) { return s + it.precio * it.q; }, 0);
  var disc = DB.config.condicionIva === "RI";
  var neto = disc ? subtotal / 1.21 : subtotal;
  var iva = subtotal - neto;
  byId("posTipo").textContent = tipoComprobante(DB.config.condicionIva, "CF");
  byId("posSub").textContent = money(neto);
  byId("posIva").textContent = disc ? money(iva) : "No discrimina";
  byId("posTotal").textContent = money(subtotal);
  byId("posCobrar").innerHTML = "COBRAR · " + money(subtotal) + ' <span class="kbd" style="color:#0E2C49">F12</span>';

  var pays = byId("posPays").querySelectorAll(".pay");
  pays.forEach(function (b) { b.classList.toggle("pay--on", b.dataset.pago === currentPago); });
  var ef = byId("posEfectivo");
  ef.style.display = currentPago === "efectivo" ? "flex" : "none";
...
function posInc(id) { var c = cart.filter(function (x) { return x.pid === id; })[0]; if (c) { c.qty += 1; renderTicket(); } }
function posDec(id) { var c = cart.filter(function (x) { return x.pid === id; })[0]; if (c) { c.qty -= 1; if (c.qty <= 0) cart = cart.filter(function (x) { return x.pid !== id; }); renderTicket(); } }
function posRemove(id) { cart = cart.filter(function (x) { return x.pid !== id; }); renderTicket(); }
function posSetCat(c) { posCat = c; renderPOS(); }
function selectPago(m) { currentPago = m; renderTicket(); }
function posVuelto() {
  var total = cart.reduce(function (s, c) { var p = prod(c.pid); return s + price(p) * c.qty; }, 0);
  var paga = parseFloat(val("posPaga")) || 0;
  byId("posVuelto").textContent = paga > total ? "Vuelto " + money(paga - total) : "";
}
function cobrar() {
  ...
  var total = items.reduce(function (s, it) { return s + it.precio * it.q; }, 0);
  var disc = DB.config.condicionIva === "RI";
  var neto = disc ? Math.round(total / 1.21) : total, iva = total - neto;
  var tipo = tipoComprobante(DB.config.condicionIva, "CF");
  DB.contador += 1; var nro = DB.contador;
  DB.ventas.push({ n: nro, hora: hhmm(), items: items, pago: currentPago, receptor: "CF" });
  ...
  var cae = caeSim();
  save();
  cart = []; renderPOS(); renderInicio();
  var body = '<div style="text-align:center;margin-bottom:8px"><div style="font-size:42px">✅</div>' +
    '<div class="strong" style="font-size:22px">' + money(total) + "</div>" +
    '<div class="muted">' + tipo + " " + DB.config.puntoVenta + "-" + pad(nro, 8) + "</div></div>" +
    '<div class="kv"><span>Forma de pago</span><b>' + pagoLabel(currentPago) + "</b></div>" +
    (disc ? '<div class="kv"><span>Neto / IVA 21%</span><b>' + money(neto) + " / " + money(iva) + "</b></div>" : "") +
    (currentPago === "efectivo" && paga > total ? '<div class="kv"><span>Paga con / Vuelto</span><b>' + money(paga) + " / " + money(paga - total) + "</b></div>" : "") +
    '<div class="kv"><span>CAE (simulado)</span><b>' + cae + "</b></div>" +
    '<div class="kv"><span>Estado</span><span class="badge badge--ok">AUTORIZADA</span></div>';
  openModal("Venta registrada", body, function () {}, "Nueva venta", "Imprimir ticket", function () { toast("Ticket enviado a la impresora (demo)", "ok"); return false; });
}
...
function renderCatalogo() {
  ...
  var rows = DB.products.filter(function (p) {
    return (ru === "Todos" || p.rubro === ru) && (p.nombre.toLowerCase().indexOf(q) >= 0 || (p.codigo || "").indexOf(q) >= 0);
  });
  byId("catalogoBody").innerHTML = rows.map(function (p) {
    var precio = lista === "mayorista" ? p.precioMay : p.precioMin;
    var util = p.costo ? Math.round((precio - p.costo) / p.costo * 100) : 0;
    return '<tr class="clickable" onclick="openProductModal(\'' + p.id + '\')">' +
      "<td>" + esc(p.codigo || "—") + '</td><td class="strong">' + esc(p.nombre) + "</td><td>" + esc(p.rubro) + "</td>" +
...
function selOpts(arr, sel) {
  return arr.map(function (o) { var v = o.length === 2 && o.push ? o : null; var value = Array.isArray(o) ? o[0] : o; var label = Array.isArray(o) ? o[1] : o; return '<option value="' + esc(value) + '"' + (value === sel ? " selected" : "") + ">" + esc(label) + "</option>"; }).join("");
}
...
function openProductModal(pid) {
  var p = pid ? prod(pid) : null;
  var rs = ["Bebidas", "Almacén", "Lácteos", "Fiambrería", "Limpieza", "Panadería", "Otros"];
  ...
  openModal(pid ? "Editar artículo" : "Nuevo artículo", body, function () {
    ...
    var costo = numv("m_costo"), u = numv("m_util");
    var precioMin = Math.round(costo * (1 + u / 100));
    ...
    if (p) { for (var k in data) p[k] = data[k]; toast("Artículo actualizado", "ok"); }
    else { data.id = "p" + Date.now(); DB.products.push(data); toast("Artículo agregado al catálogo", "ok"); }
...
function renderStock() {
  ...
  byId("stockBody").innerHTML = rows.map(function (p) {
    return '<tr><td class="strong">' + esc(p.nombre) + "</td><td>" + (p.unidad === "kg" ? "Fiambrería" : "Salón") + "</td>" +
      '<td class="num">' + qtyFmt(p.stock) + '</td><td class="num">' + qtyFmt(p.minimo) + "</td>" +
      "<td>" + (p.unidad === "kg" ? "Fraccionado" : "—") + "</td><td>" + estadoStock(p) + "</td></tr>";
...
function ingresoStock() {
  var body = selectHtml("s_prod", "Artículo", DB.products.map(function (p) { return [p.id, p.nombre]; }), DB.products[0].id) + fieldHtml("s_cant", "Cantidad a ingresar", 1, "number");
  openModal("Ingreso por compra", body, function () {
    var p = prod(val("s_prod")), c = numv("s_cant"); if (c <= 0) { toast("Cantidad inválida", "warn"); return false; }
    p.stock = +(p.stock + c).toFixed(3); save(); renderStock(); renderCatalogo(); renderPOS(); renderInicio();
    toast("Ingresaron " + qtyFmt(c) + " de " + p.nombre, "ok");
  }, "Ingresar");
}
function ajusteStock() {
  var body = selectHtml("s_prod", "Artículo", DB.products.map(function (p) { return [p.id, p.nombre]; }), DB.products[0].id) + fieldHtml("s_nuevo", "Stock real (ajuste)", DB.products[0].stock, "number");
  openModal("Ajuste manual de stock", body, function () {
    var p = prod(val("s_prod")); p.stock = Math.max(0, numv("s_nuevo")); save(); renderStock(); renderCatalogo(); renderPOS(); renderInicio();
...
function renderCaja() {
  ...
  var e = byId("cajaEstado");
  e.className = "badge " + (DB.caja.abierta ? "badge--ok" : "badge--n");
  e.textContent = DB.caja.abierta ? "Turno abierto" : "Turno cerrado";
  var b = byId("cajaArqueoBtn");
  if (DB.caja.abierta) { b.textContent = "Arqueo y cierre"; b.onclick = cajaArqueo; } else { b.textContent = "Abrir caja"; b.onclick = cajaAbrir; }
  byId("cajaMovs").innerHTML = DB.caja.movimientos.slice().reverse().map(function (m) {
    return "<tr><td>" + m.hora + "</td><td>" + esc(m.concepto) + "</td><td>" + cajaBadge(m.tipo) + '</td><td class="num">' + (m.tipo === "egreso" ? "– " : "") + money(m.monto) + "</td></tr>";
  }).join("");
}
function cajaMov(tipo) {
  if (!DB.caja.abierta) { toast("La caja está cerrada", "warn"); return; }
  var body = fieldHtml("k_concepto", "Concepto", "") + fieldHtml("k_monto", "Monto $", 0, "number");
  openModal(tipo === "ingreso" ? "Ingreso a caja" : "Egreso de caja", body, function () {
...
function cajaArqueo() {
  ...
  var body = '<div class="kv"><span>Saldo teórico</span><b>' + money(teorico) + "</b></div>" + fieldHtml("k_fisico", "Conteo físico $", teorico, "number");
  openModal("Arqueo y cierre de turno", body, function () {
    var fis = numv("k_fisico"), dif = fis - teorico;
    DB.caja.abierta = false; save(); renderCaja();
    toast(dif === 0 ? "Caja cerrada · sin diferencias" : "Caja cerrada · diferencia " + money(dif), dif === 0 ? "ok" : "warn");
  }, "Cerrar turno");
}
function cajaAbrir() {
  var body = fieldHtml("k_ini", "Monto inicial $", 50000, "number");
  openModal("Abrir caja", body, function () {
    DB.caja.abierta = true; DB.caja.movimientos = [{ hora: hhmm(), concepto: "Apertura de caja", tipo: "apertura", monto: numv("k_ini") }];
    save(); renderCaja(); toast("Caja abierta", "ok");
  }, "Abrir");
}
...
function renderCuentas() {
  if (DB.ui.ctaTab === "proveedores") {
    byId("ctacteBody").innerHTML = '<tr><td colspan="6" class="muted" style="text-align:center;padding:26px">Sin proveedores cargados en la demo</td></tr>';
    return;
  }
  byId("ctacteBody").innerHTML = DB.clients.map(function (c) {
    return '<tr><td class="strong">' + esc(c.nombre) + "</td><td>" + esc(c.cuit) + "</td><td>" + esc(c.condicion) + "</td>" +
      '<td class="num strong"' + (c.saldo > 0 ? ' style="color:var(--danger)"' : "") + ">" + money(c.saldo) + "</td>" +
      '<td class="num">' + money(c.limite) + "</td><td>" + ctaEstado(c) + "</td></tr>";
  }).join("");
}
...
function registrarCobro() {
  ...
  var body = selectHtml("cb_cli", "Cliente", conDeuda.map(function (c) { return [c.id, c.nombre + " (debe " + money(c.saldo) + ")"]; }), conDeuda[0].id) + fieldHtml("cb_monto", "Monto del cobro $", 0, "number");
  openModal("Registrar cobro", body, function () {
    var c = DB.clients.filter(function (x) { return x.id === val("cb_cli"); })[0], m = numv("cb_monto");
    if (m <= 0) { toast("Monto inválido", "warn"); return false; }
    c.saldo = Math.max(0, c.saldo - m);
    DB.caja.movimientos.push({ hora: hhmm(), concepto: "Cobro Cta. Cte. — " + c.nombre, tipo: "ingreso", monto: m });
    save(); renderCuentas(); renderCaja(); toast("Cobro registrado", "ok");
  }, "Cobrar");
}
function nuevoCliente() {
  var body = fieldHtml("cl_nombre", "Nombre / Razón social", "") +
    ...
    fieldHtml("cl_limite", "Límite de crédito $", 50000, "number");
  openModal("Nuevo cliente", body, function () {
...
function renderReportes() {
  ...
  DB.ventas.forEach(function (v) { byPago[v.pago] = (byPago[v.pago] || 0) + ventaTotal(v); });
  ...
  DB.ventas.forEach(function (v) {
    ...
    var o = byTipo[t] || (byTipo[t] = { c: 0, neto: 0, iva: 0, total: 0 });
    o.c++; o.neto += neto; o.iva += iva; o.total += total;
  });
...
function setCondicion(c) {
  DB.config.condicionIva = c; save();
  renderConfig(); renderPOS(); renderReportes();
  toast("Condición fiscal: " + (c === "RI" ? "Responsable Inscripto" : "Monotributo"), "ok");
}
function toggleSwitch(el, key) { el.classList.toggle("off"); if (key) { DB.config[key] = !el.classList.contains("off"); save(); } }
function resetDemo() {
  openModal("Reiniciar la demostración", '<p class="muted" style="margin:0">Se borran los cambios (ventas, artículos, stock, caja) y vuelve a los datos de ejemplo. ¿Confirmás?</p>', function () {
...
function enviarIA() {
  ...
  var body = byId("iaBody");
  body.innerHTML += '<div class="msg msg--me">' + esc(t) + "</div>";
  var totalDia = DB.ventas.reduce(function (s, v) { return s + ventaTotal(v); }, 0);
  body.innerHTML += '<div class="msg msg--ia">Según tus datos cargados: llevás <b>' + DB.ventas.length + "</b> comprobantes por <b>" + money(totalDia) + "</b> hoy. <span class=\"muted\">(respuesta simulada · solo lectura)</span></div>";
...
function openModal(title, bodyHtml, onConfirm, confirmLabel, cancelLabel, onCancel) {
  ...
  byId("modalConfirm").textContent = confirmLabel || "Guardar";
  byId("modalCancel").textContent = cancelLabel || "Cancelar";
  modalOk = onConfirm || null; modalCancelFn = onCancel || null;
  byId("modal").classList.add("modal--show");
}
...
load();
(function initRubros() {
  var sel = byId("catRubro");
  if (sel) sel.innerHTML = '<option value="Todos">Rubro: todos</option>' + selOpts(rubros(), "Todos");
})();
document.getElementById("nav").addEventListener("click", function (e) {
  ...
  byId("pageTitle").textContent = item.dataset.title;
  byId("pageCrumb").textContent = item.dataset.crumb;
...
// Logo real (si carga assets/logo.png, oculta el provisorio)
document.querySelectorAll(".logo__img").forEach(function (img) {
  function ok() { var l = img.closest(".logo"); if (l) l.classList.add("has-logo"); }
  if (img.complete && img.naturalWidth > 0) ok(); else img.addEventListener("load", ok);
});
